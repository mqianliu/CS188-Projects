...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        y1 = nn.ReLU(nn.AddBias(xw1, self.b1))
        xw2 = nn.Linear(y1, self.w2)
        y2 = nn.ReLU(nn.AddBias(xw2, self.b2))
        xw3 = nn.Linear(y2, self.w3)
        y3 = nn.AddBias(xw3, self.b3)
        return y3
        ...
        while converge is False:
            ...
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                if nn.as_scalar(loss) > 0.001:
                    ...
                    break
                para = [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3]
                grad_list = nn.gradients(loss, para)
                # print(grad_wrt_m)
                for i in range(len(para)):
                    para[i].update(grad_list[i], self.alpha)
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        y2 = nn.ReLU(nn.AddBias(xw2, self.b2))
        xw3 = nn.Linear(y2, self.w3)
        y3 = nn.ReLU(nn.AddBias(xw3, self.b3))
        xw4 = nn.Linear(y3, self.w4)
        y4 = nn.AddBias(xw4, self.b4)
        return y4
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.batch_size = 30
        self.size = 256
        self.w_in = nn.Parameter(self.num_chars, self.size)
        self.w_hidden = nn.Parameter(self.size, self.size)
        self.w_out = nn.Parameter(self.size, 5)
    ...
    def run(self, xs):
        ...
        for i in range(0, len(xs)):
            self.h = nn.ReLU(nn.Add(nn.Linear(xs[i], self.w_in), nn.Linear(self.h, self.w_hidden)))
    ...
    def train(self, dataset):
        ...
        while converge is False:
            ...
            for x, y in dataset.iterate_once(self.batch_size):
                loss = self.get_loss(x, y)
                para = [self.w_in, self.w_hidden, self.w_out]
                grad_list = nn.gradients(loss, para)
                for i in range(len(para)):
                    para[i].update(grad_list[i], self.alpha)
